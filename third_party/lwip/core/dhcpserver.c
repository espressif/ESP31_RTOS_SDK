...
extern struct netif * wifi_get_netif(WIFI_INTERFACE if_index);
...
//									os_printf("the ip addr has been request\n");
						...
						bzero(pdhcps_pool, sizeof(struct dhcps_pool));
...
//							client_address_plus.addr = dhcps_lease.start_ip.addr;
						...
						extern bool  wifi_softap_set_station_info(uint8 *bssid, struct ip_addr *ip);
									...
									u16_t port)
{
	    ...
	    bzero(pmsg_dhcps, sizeof(struct dhcps_msg));
	    if (NULL == pmsg_dhcps){
	    	pbuf_free(p);
	    	return;
	    }
