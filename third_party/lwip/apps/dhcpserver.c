...
//static const u32_t magic_cookie ICACHE_RODATA_ATTR = 0x63538263;
//static const char magic_cookie[] ICACHE_RODATA_ATTR STORE_ATTR = {0x63,0x82,0x53,0x63};
...
extern bool tcpip_dep_get_ip_info(WIFI_INTERFACE wifi_if, struct ip_info *if_ip);
...
static u8_t* add_offer_options(u8_t *optptr)
{
        ...
        *optptr++ = ip4_addr4( &ipadd);
		...
		tcpip_dep_get_ip_info(SOFTAP_IF, &if_ip);
			...
			if (client_address.addr > dhcps_lease.end_ip.addr) {
			    ...
			    pnode = NULL;
        			...
        			pdhcps_pool = (struct dhcps_pool *)malloc(sizeof(struct dhcps_pool));
                            memset(pdhcps_pool ,0x00 ,sizeof(pdhcps_pool));
					...
					u16_t port)
{
	    ...
	    pmsg_dhcps = (struct dhcps_msg *)malloc(sizeof(struct dhcps_msg));
           memset(pmsg_dhcps ,0x00 ,sizeof(pmsg_dhcps));
...
//  NOT USE
	...
	if(please->enable) {
        	...
        	memset(&ip_info, 0x00, sizeof(struct ip_info));
              tcpip_dep_get_ip_info(SOFTAP_IF, &info);
...
bool wifi_softap_get_dhcps_lease(struct dhcps_lease *please)
{
	...
	if (NULL == please)
		return false;
	if (dhcps_lease.enable == FALSE){
		if (get_dhcps_status() == DHCP_STOPPED)
			return false;
	} else {
	    ;
	}
